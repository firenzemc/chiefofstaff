...
class AuditStore:
    ...
    Usage::
        ...
        store = AuditStore("mainframe_audit.db")
        await store.init()          # creates tables if needed
    ...
    async def get_runs_by_meeting(self, meeting_id: str) -> list[PipelineRun]:
        ...
        return [self._row_to_run(r, cursor.description) for r in rows]
        ...
        await self._db.execute(
            """INSERT OR REPLACE INTO execution_records
               (record_id, meeting_id, run_id,
                ...
                risk_level, risk_reason, approved_by, approved_at,
    ...
    async def get_execution(self, record_id: str) -> Optional[ExecutionRecord]:
        ...
        if row is None:
            return None
        return self._row_to_exec(row, cursor.description)
    ...
    async def get_executions_by_meeting(self, meeting_id: str) -> list[ExecutionRecord]:
        ...
        rows = await cursor.fetchall()
        return [self._row_to_exec(r, cursor.description) for r in rows]
    ...
    async def update_execution_status(
        ...
        **kwargs,
        ...
        """Update status and any extra fields (executed_at, approved_by, etc.)."""
        sets = ["status = ?"]
        params: list = [status.value]

        for col in ("executed_at", "approved_by", "approved_at",
    ...
    def _row_to_exec(cls, row, description) -> ExecutionRecord:
        ...
        return ExecutionRecord(
            ...
            source_speaker=d["source_speaker"],
